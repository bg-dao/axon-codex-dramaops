...
if [[ "$(uname -s)" != "Darwin" || "$(uname -m)" != "arm64" ]]; then
  echo "SceneOps v0.1 macOS release builds require an Apple Silicon Mac." >&2
  exit 1
fi
...
app_path="$repo_root/build/bin/SceneOps.app"
...
if [[ -n "${APPLE_NOTARY_PROFILE:-}" && -n "${APPLE_SIGN_IDENTITY:-}" ]]; then
  archive_path="$repo_root/build/bin/SceneOps-notarization.zip"
